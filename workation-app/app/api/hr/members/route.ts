...
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await db.from('users').select('id,name,email,role,status,company_id,company_name').eq('role','emp').eq('company_id',company.id).in('status',['pending','approved']).order('name')
...
export async function POST(request: Request) {
  try {
    ...
    const { data: updated, error: updateError } = await db.from('users').update({status:'approved'}).eq('id',employee.id).eq('role','emp').eq('company_id',company.id).eq('status','pending').select('id').maybeSingle()
