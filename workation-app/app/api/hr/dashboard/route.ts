...
export async function GET(request: Request) {
  try {
    ...
    const { data: employeeRows, error: employeeError } = await db.from('users').select('id,name,email,status').eq('role', 'emp').eq('company_name', company).order('name')
    if (employeeError) throw new MembershipError('직원 정보를 불러오지 못했습니다.')
    const members = employeeRows || []
    const ids = members.map((member) => member.id)
    if (!ids.length) return NextResponse.json({ company, members, bookings: [], reports: [] }, { headers: { 'Cache-Control': 'no-store' } })
    const [{ data: bookingRows, error: bookingError }, { data: reportRows, error: reportError }] = await Promise.all([
      db.from('bookings').select('id,user_id,accommodation_id,start_date,end_date,guests,total_price,status,created_at,payment_type').in('user_id', ids).order('created_at', { ascending: false }),
