...
export type Membership = { id: string; name: string; email: string; role: string; status: string; company_name: string | null }
...
export async function authenticatedMember(request: Request, db: SupabaseClient) {
  ...
  const { data, error } = await db.from('users').select('id,name,email,role,status,company_name').eq('id', auth.user.id).maybeSingle()
...
export function canApproveEmployee(hr: Membership, employee: Membership) {
  return hr.role === 'hr' && hr.status === 'approved' && Boolean(hr.company_name?.trim()) && employee.role === 'emp' && employee.company_name === hr.company_name && employee.id !== hr.id
}
